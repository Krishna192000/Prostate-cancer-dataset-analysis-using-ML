...
def FS_RF():
    print("\n\n------------Starting run of gleason_3_FS_RF------------\n")
    print(
        "This file will read the filtered 25k fetures and apply \nFeature Selection with Random forest and output top600 features.\n")

    data = pd.read_csv('Gleason_score/gleason_Data_2_Low_Variance.csv')

    X = np.array(data.drop(['Unnamed: 0','GLEASON_SCORE', 'CLIN_T_STAGE', 'PATH_T_STAGE', 'gleason_score'],1))
    ...
    data.to_csv('Gleason_score/gleason_Data_3_FS_RF_top600.csv', sep=',')

    print("\n------------Ending run of gleason_3_FS_RF------------")
